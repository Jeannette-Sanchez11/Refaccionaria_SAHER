...
public class Insert {
    ...
    public boolean insertCompra(String rfc_Proveedor, float monto_TotalC, String fecha_Compra) {//metodo para insertar un usuario
        //se prepara la insercion del dato de usuario
        sql = "insert into CompraM(rfc_Proveedor,monto_TotalC,fecha_Compra) values('" + rfc_Proveedor + "'," + monto_TotalC + ",'" + fecha_Compra + "');";
        respuesta = true;// nos devuelven el valor booleano despues de la insercion
        try {
            st = conex.createStatement();
            respuesta = st.execute(sql);
